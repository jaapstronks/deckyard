...
async function render() {
  ...
  try {
    ...
    try {
      ...
      // A `?locale=` URL param is an explicit per-session choice and
      ...
      // uiLocale is 'en' and would otherwise override a deep-linked ?locale=nl).
      if (
        !getSessionLocaleOverride() &&
...
async function bootstrap() {
  initUiMode();
  try {
    // A `?locale=` URL param (validated against the manifest) wins over
