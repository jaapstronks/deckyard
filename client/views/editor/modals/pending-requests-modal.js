 ...
 */
export function showPendingRequestsModal({
  h,
  root,
  toast,
  requests,
  ...
  const modalApi = openModal(h, root, {
    ...
    hint: t(
      'editor.requestsHelp',
      'Accept to release your lock and let them edit.',
    ),
  ...
  for (const req of requests) {
    ...
    const hasValidName =
      ...
      req.requesterName.trim();
    const displayName = hasValidName ? req.requesterName : null;

    const itemEl = h('div', {
      class: 'lock-request-item',
      style:
        'padding: 12px 16px; background: var(--bg-secondary, #f5f5f5); border-radius: 8px; margin-bottom: 8px;',
    });
    itemEl.append(
      h(
        'div',
            ...
            h('button', {
              ...
              onclick: async () => {
                ...
                if (result?.ok) {
                  modalApi.close();
                  toast.success(
                    t('editor.accessGrantedTo', 'Access granted. Reloading...'),
                  );
      ...
      req.message && req.message !== 'null'
        ? h('div', {
            style:
              'color: var(--text-muted); margin-top: 8px; font-style: italic;',
            text: `"${req.message}"`,
          })
        : null,
    );
