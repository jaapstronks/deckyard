  ...
  for (const field of fields) {
    ...
    if (field.type === 'items' && field.itemFields) {
      if (field.minItems != null)
        lines.push(`- **Min items:** ${field.minItems}`);
      if (field.maxItems != null)
        lines.push(`- **Max items:** ${field.maxItems}`);
      lines.push('- **Item fields:**');
      for (const itemField of field.itemFields) {
        const itemReq = itemField.required ? ' (required)' : '';
        const itemMaxLen = itemField.maxLength
          ? ` [max: ${itemField.maxLength}]`
          : '';
        lines.push(
          `  - \`${itemField.key}\`: ${itemField.type}${itemReq}${itemMaxLen}`,
        );
        if (itemField.type === 'enum' && itemField.options) {
          const itemOpts = itemField.options.map((o) =>
...
export function openJsonDebugModal({
    ...
    if (editedJson.id !== slide.id) {
      const ok = await confirmModal(document.body, {
        ...
        message: t(
          ...
          'Warning: You are changing the slide ID. This may cause issues. Continue?',
        ),
    ...
    if (editedJson.type !== slide.type) {
      const ok = await confirmModal(document.body, {
        ...
        message: t(
          ...
          'Warning: You are changing the slide type. The content may not render correctly. Continue?',
        ),
        confirmLabel: t('common.continue', 'Continue'),
        danger: true,
      });
  ...
  schemaCopyBtn.onclick = async () => {
    try {
      ...
      toast.success(
        t('admin.jsonDebug.schemaCopied', 'Schema copied to clipboard'),
      );
    } catch (e) {
      toast.error(t('admin.jsonDebug.copyFailed', 'Failed to copy'));
    }
  ...
  return (
    markdown
      ...
      .replace(
        /```json\n([\s\S]*?)```/g,
        (_, code) =>
          `<pre class="json-debug-code">${highlightJson(code.trim())}</pre>`,
      )
