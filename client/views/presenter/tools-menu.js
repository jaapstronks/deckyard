...
export function createPresenterToolsMenu({
  ...
  const toolsBtn = h('button', {
    ...
    title: t(
      'presenter.tools.title',
      'Session tools (notes / copy link / follow code)',
    ),
    type: 'button',
  });
  ...
  const presenterSectionHint = h('div', {
    ...
    text: t(
      'presenter.tools.presenterHint',
      'Speaker notes view, for you on a second device.',
    ),
  });
  const presenterSectionWarn = h('div', {
    class: 'presenter-tools-warn',
    text: t(
      'presenter.tools.presenterWarn',
      'This link can control your live deck. Do not send it to your audience; use the audience link below.',
    ),
  });
  ...
  const presenterCopyBtn = h('button', {
    ...
    onclick: async () => {
      ...
      const confirmed = await confirmModal(document.body, {
        ...
        confirmLabel: t(
          'presenter.tools.copyPresenterConfirmBtn',
          'Copy presenter link',
        ),
        danger: true,
      });
      ...
      await copyText?.(
        t(
          'presenter.tools.copyPresenterPrompt',
          'Copy presenter companion link:',
        ),
        u.toString(),
      );
    },
  ...
  const audienceSectionHint = h('div', {
    class: 'help presenter-tools-hint',
    text: t(
      'presenter.tools.audienceHint',
      'Public follow-along view (slides + Q&A + interactions). Safe to share with everyone.',
    ),
  });
