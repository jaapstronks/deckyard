 ...
 * `.slide-icon-card-grid::after`, an alpha `linear-gradient`.
 *
 * ## The `::before` gradient layer, and the scope bug that used to hide it
 *
 * Until 2026-07-31 the `::before` sibling of that layer was a special case: it
 * painted nothing anywhere, so there was nothing to rasterize even with the gate
 * forced open. `--t-slide-gradient-bg` was declared on `.ps-theme` (the stage)
 * while its value referenced `var(--g1x)` … `var(--g3y)`, which only exist on the
 * **slide root**. A custom property's `var()` references are substituted where
 * that property is *computed* — on the stage, where `--g1x` did not exist — so the
 * result was guaranteed-invalid, inherited down as invalid, and
 * `background: var(--t-slide-gradient-bg, transparent)` fell back to nothing.
 *
 * `themeVarsCssText()` now declares that one var on the slide root instead
 * (`SLIDE_ROOT_VARS` in `server/utils/themes.js`), so the layer paints wherever
 * the gate lets it — with the per-slide jitter from `gradientVarsForSlide()`
 * actually reaching the gradient. For upstream exports nothing changes: every
 * export path still sets `--t-gradient-enabled: 0`, so the layers compute to
 * `opacity: 0` and the probe skips them. A fork that opens that gate now gets
 * these layers rasterized like any other, one bitmap per distinct position.
 *
 ...
 * intentionally not matched: it decorates `::before` layers rather than the slide
 * root, and those are decided by measuring the assembled document (see the module
 * comment), never by reading CSS text. The `-text` / `-accent` companions never
 * hold a gradient anyway.
 */
const SLIDE_BG_VAR_RE = /^--t-slide-bg-[a-z0-9-]+$/;

/** A CSS function that names an external bitmap. */
const IMAGE_FN_RE = /\b(?:url|image-set|image)\(/i;

/**
 * `--name: value;` inside the theme's generated vars CSS. The `url("…")`
 * alternative matters because a data URL contains a `;`, so a plain `[^;}]+`
 * would cut such a declaration in half.
 *
 * Deliberately NOT extended to single-quoted `url('…')`, which is what the
 * embed pass writes now that it runs over this CSS: a value carrying an image
 * function is dropped by {@link findGradientBgVars} before anything reads it,
 * and the rewrite below only touches vars that got that far. So a partial
 * capture of such a value has no consumer, and widening the pattern would add
 ...
 *
 * A value that stacks a gradient *over an image* is deliberately left alone.
 * A theme variant whose design is artwork with a legibility scrim on top —
 * `linear-gradient(…), url('…/bg.png') center / cover, #13393a` — is a real
 * combination, and rasterizing it would destroy it twice over: the raster page
 * runs offline (see `openRasterPage`, the SSRF guard), so the image cannot
 * load and the var would be rewritten to its bare fallback colour with the
 * artwork silently gone; and even if it loaded, baking a photograph into a
 * {@link gradientRasterWidth}-wide bitmap is not a trade a soft gradient wash
 * makes but a photo does not. Such a value keeps its live CSS: the export is
 * slower by the cost of one image, and correct.
 *
 * The test is for an image *function*, not for the spelling `url(`:
 * `image-set()` and `image()` name an external bitmap just as well, and
 * matching only one of them would leave the same bug one spelling over.
 *
 * @param {string} themeVarsCss - The theme CSS generated by `themeVarsCssText()`.
...
export function findGradientBgVars(themeVarsCss) {
  ...
  for (const m of String(themeVarsCss || '').matchAll(VAR_DECL_RE)) {
    ...
    if (IMAGE_FN_RE.test(value)) continue;
    out.set(name, value);
  }
  return out;
}
 ...
 * could resolve its own `var()` references at all, which depends on where it was
 * declared (see the module comment).
 *
 * @param {Object} opts
 ...
 *
 * @param {Object} opts
 * @param {string} opts.themeVarsCss - The theme CSS generated by `themeVarsCssText()`.
 * @param {string[]} opts.slidesHtml - Rendered slide HTML, in page order.
