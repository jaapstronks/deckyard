 ...
 * would cut such a declaration in half. Both quote styles are accepted: this
 * module writes `url("…")` for its own bitmaps, while `embedLocalCssUrls`
 * (which now runs over this CSS, so a theme's local artwork survives to the
 * export) writes `url('…')`.
 */
const VAR_DECL_RE =
  /(--t-slide-bg-[a-z0-9-]+)\s*:\s*((?:url\("[^"]*"\)|url\('[^']*'\)|[^;}])+)/g;
 ...
 * runs offline (see `openRasterPage`, the SSRF guard), so the `url()` cannot
