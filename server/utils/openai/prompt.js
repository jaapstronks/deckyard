...
export function summarizeDeckForPrompt(deck, { maxSlides = 60 } = {}) {
  ...
  for (let i = 0; i < shown.length; i += 1) {
    const s = shown[i] || {};
    const type = truncateForPrompt(s?.type || '', 60) || '(missing type)';
    ...
    if (typeof c.layout === 'string' && c.layout.trim())
      summaryBits.push(`layout="${truncateForPrompt(c.layout, 40)}"`);
    if (typeof c.variant === 'string' && c.variant.trim())
      summaryBits.push(`variant="${truncateForPrompt(c.variant, 40)}"`);
    if (Array.isArray(c.items) && c.items.length)
      summaryBits.push(`items=${c.items.length}`);
    if (typeof c.chartType === 'string' && c.chartType.trim())
      summaryBits.push(`chartType="${truncateForPrompt(c.chartType, 20)}"`);
    if (typeof c.label === 'string' && c.label.trim())
      summaryBits.push(`label="${truncateForPrompt(c.label, 120)}"`);
    ...
    if (typeof c.image === 'string' && c.image.trim())
      summaryBits.push(`image="${truncateForPrompt(c.image, 120)}"`);
    const bgImg =
      (typeof c.slideBgImage === 'string' && c.slideBgImage.trim()) ||
      (typeof c.bgImage === 'string' && c.bgImage.trim());
    if (bgImg)
      summaryBits.push(`slideBgImage="${truncateForPrompt(bgImg, 120)}"`);

    const bitStr = summaryBits.length ? ` — ${summaryBits.join(' · ')}` : '';
