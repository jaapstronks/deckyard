 ...
 *
 * On a single-organization instance the instance admin is the only administrator
 ...
 * time. In multi-organization mode the active membership was already read while
...
export async function resolveDesignerCapability(user) {
  ...
  if (!isMultiOrgEnabled() && user.isAdmin) return true;
