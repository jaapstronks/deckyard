 ...
 */
function buildSlideIterationPrompt({ command, strategy, lang, catalogPrompt }) {
 ...
 */
function buildDeckIterationPrompt({ command, strategy, lang, catalogPrompt }) {
  const langLabel = lang === 'nl' ? 'DUTCH' : 'ENGLISH';
...
export async function iterateSlide(slide, command, {
  ...
  const systemPrompt = buildSlideIterationPrompt({
    command,
    strategy,
    lang,
    catalogPrompt,
...
export async function iterateDeck(deck, command, {
  ...
  const systemPrompt = buildDeckIterationPrompt({
    command,
    strategy,
    lang,
    catalogPrompt,
