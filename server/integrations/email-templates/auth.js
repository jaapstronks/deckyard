...
export function buildPasswordResetEmail({ tr, name, resetUrl }) {
...
export function buildUserInvitationEmail({ tr, name, inviter, setupUrl }) {
  const greeting = tr('email.common.greeting', 'Hi {name},', { name });
  const bodyText = tr(
...
export function buildActivationReminderEmail({ tr, name, inviter, setupUrl }) {
  const greeting = tr('email.common.greeting', 'Hi {name},', { name });
  const bodyText = tr(
