...
/**
 * GET /api/slide-types — slide type metadata for the editor UI. Keeps
 * client/server in sync. Merges registered types (core + file-based) with
 * published custom types.
 *
...
async function handleSlideTypeList({ storageScope, res, authedUser }) {
  ...
  for (const [key, def] of Object.entries(SLIDE_TYPES)) {
    meta[key] = {
      ...
      // re-derived when it is copied (duplicate / paste / library insert) or
      // filled in when it is saved. Same seam half as `inspectorKeeps`: the
      // editor's clone helper reads this response, so a fork type declaring an
      // id of its own is only heard if the declaration travels. See
      // shared/slide-types/instance-keys.js.
      instanceKeys: (() => {
        const r = slideInstanceKeys(def);
...
/**
 * Declarative route table for `/api/slide-types`, dispatched through the shared
 * {@link dispatchRoutes} (A7.19 C8). Method mismatch falls through to the next
 * mount (the original chain had no 405 here).
 *
 * @type {import('../../utils/router.js').Route[]}
 */
export const ROUTES = [
  { method: 'GET', pattern: '/api/slide-types', handler: handleSlideTypeList },
];
