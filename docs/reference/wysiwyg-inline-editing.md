...
- `skipWhenAbsent` is load-bearing for text-blocks, the last type with a
  numbered mirror: removing it would let an add on a legacy deck silently
  switch the renderer's data source.
