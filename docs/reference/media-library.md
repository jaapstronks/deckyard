...
| `IMAGEKIT_HIDDEN_TAGS`                                                              | `media/imagekit.js`              | Optional, comma-separated. Files carrying one of these tags are left out of the picker listing, its search and the tag suggestions; lookups by file id still resolve them, so slides that use one keep working. Unset hides nothing. |
