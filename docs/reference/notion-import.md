...
| `NOTION_FEATURE` | Turns on the flagged wizard endpoints (`subjects`, `compose`, `suggest`) via `enableNotion` in `server/config/feature-flags.js`. Forced off in demo mode. |
