...
- **Rendering pipeline** for actual slides goes through the server: `slide-render.js` routes a type to `/api/presentations/:id/render-slide` when it is either **not bundled** (the normal case for a custom type under a new name) or **bundled but overridden server-side** (a fork file in `custom/slide-types/` that replaces a core name with `override: true`, named in `window.__DECK_SERVER_RENDERED_TYPES__`). Server uses compiled templates. Types on the tombstone record are excluded — they render the archived-slide placeholder client-side, so asking would strand them on the loading box when there is no presentation id.
