...
Image collection walks slide content — every slide of the deck and of each i18n version — through `collectServedAssetRefs` in `shared/slide-types/deck-assets.js`, the same walk the [`.deck` bundle](./deck-bundle-format.md) uses. It is deep and key-agnostic: a ref is recognised by its own shape, so a new or nested image field is picked up without a list to maintain. The refs it takes are the paths this installation serves (`/uploads/`, `/assets/`, `/custom/assets/`, `/custom/themes/`) — a deck's `/custom/…` theme background counts, a remote `http(s)` URL does not, since a bare string cannot say whether it is an image or a link target and a remote URL is still valid after a restore. Image-library entries and theme logos are added directly from their own records and may be remote. Local paths are read from disk with a path-traversal guard — the resolver accepts exactly the class the collector produces (`isServedAssetRef`), and uploads come from the configured uploads directory (`UPLOADS_DIR` / sandbox aware, like the `.deck` bundle); remote URLs are fetched with a 30s timeout and a concurrency limit of 5 (10 when >50 URLs). Failures are counted in `manifest.stats` and listed in `manifest.warnings`, never fatal. Note: `stats.totalSizeBytes` is set after zipping, so it appears in API responses but not in the in-ZIP `manifest.json`.
