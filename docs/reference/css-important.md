...
| **Reduced-motion resets** | `40-quote`, `82-auto-advance`, `30-chapter-title`, `10-payoff`, `52-morph-transition`, `50-presenter-layout` | 10 | `@media (prefers-reduced-motion: reduce)` forcing `animation/transition/transform: none`. Must beat keyframe and inline animation — the WCAG-recommended shape. |
