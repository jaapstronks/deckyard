...
export function overallScore(scores) {
  const values = RUBRIC_DIMENSIONS.map((d) => scores[d]).filter((n) =>
    Number.isFinite(n),
  );
...
export async function findComparableRun(
  history,
  caseIds,
  generationVendor = 'claude',
) {
  const wanted = [...caseIds].sort();
  const target = wanted.join(',');
  // A deck generated by a different vendor is a different experiment, not a
  // later data point in the same one, so never diff across vendors.
  const sameVendor = (entry) =>
    (entry.generationVendor || 'claude') === generationVendor;

  for (let i = history.length - 1; i >= 0; i -= 1) {
    if (!sameVendor(history[i])) continue;
    if ([...(history[i].caseIds || [])].sort().join(',') === target)
      return history[i];
  }

  for (let i = history.length - 1; i >= 0; i -= 1) {
    const entry = history[i];
    if (!sameVendor(entry)) continue;
...
async function narrowRunToCases(entry, caseIds) {
  ...
  try {
    const raw = await fs.readFile(
      path.join(RUNS_DIR, entry.runId, 'run.json'),
      'utf8',
    );
    run = JSON.parse(raw);
  } catch {
  ...
  const subset = (run.results || []).filter((result) =>
    caseIds.includes(result.caseId),
  );
...
export function computeDeltas(current, previous) {
  ...
  for (const dimension of [...RUBRIC_DIMENSIONS, REFERENCE_DIMENSION]) {
    ...
    out.push({
      ...
      delta,
      direction:
        Math.abs(delta) < SIGNIFICANCE_THRESHOLD
          ? 'flat'
          : delta > 0
            ? 'up'
            : 'down',
...
export async function writeReport(run, previous, outputPath) {
  ...
  lines.push(
    `- **Generation**: \`${run.generationVendor || 'claude'}\` / \`${run.generationModel || run.model}\``,
  );
  lines.push(`- **Judge**: \`${run.model}\` (effort: ${run.effort})`);
  ...
  if (previous) {
    const changed = changedPromptFiles(
      previous.promptVersion?.files,
      run.promptVersion.files,
    );
    lines.push(
  ...
  if (Number.isFinite(overall)) {
    lines.push(
      `| **Overall** | **${overall.toFixed(2)}** | ${formatDelta(overallDelta(run, previous))} |`,
    );
  }
  lines.push('');
  ...
  if (regressions.length) {
    ...
    for (const r of regressions) {
      lines.push(
        `> - ${DIMENSION_LABELS[r.dimension]} (${r.delta.toFixed(2)})`,
      );
    }
    lines.push('');
  }
  ...
  const probes = run.results.filter(
    (r) => r.repeats?.[0]?.specialTypes?.expected,
  );
  if (probes.length) {
  ...
  if (weakest.length && !run.dryRun) {
    ...
    for (const dimension of weakest) {
      lines.push(
        `### ${DIMENSION_LABELS[dimension]} (${scores[dimension].toFixed(2)})`,
      );
      lines.push('');
      for (const result of run.results) {
        ...
        if (!entry) continue;
        lines.push(
          `- **${result.caseId}** (${entry.score}/5): ${entry.rationale}`,
        );
      }
      lines.push('');
    }
  ...
  lines.push('| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |');
  for (const bucket of Object.values(run.cost.byCategory)) {
    lines.push(
      `| ${bucket.category} | \`${bucket.model}\` | ${bucket.calls} | ${bucket.inputTokens} | ` +
        `${bucket.outputTokens} | ${bucket.cacheWriteTokens} | ${bucket.cacheReadTokens} | ` +
        `$${bucket.usd.toFixed(4)} |`,
    );
  }
  lines.push(
