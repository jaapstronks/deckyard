 ...
 */
export async function findComparableRun(history, caseIds, generationVendor = 'claude') {
  ...
  const sameVendor = (entry) => (entry.generationVendor || 'claude') === generationVendor;
...
export async function writeReport(run, previous, outputPath) {
  ...
  lines.push('| Category | Model | Calls | Input | Output | Cache write | Cache read | USD |');
  lines.push('| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |');
