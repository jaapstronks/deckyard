...
function seed() {
  ...
  db = createFakeDb({
    ...
    presentation_share_links: [
      ...
      linkRow({ id: 'link-password', token: 'tok-password', password_hash: passwordHash }),
      ...
      linkRow({ id: 'link-open', token: 'tok-open', permission: 'comment', registration_mode: 'open' }),
      linkRow({ id: 'link-invite', token: 'tok-invite', permission: 'comment' }),
      linkRow({ id: 'link-other', token: 'tok-other', presentation_id: 'deck-other' }),
    ],
    share_link_guests: [],
...
async function call(method, path, { body, cookie } = {}) {
  ...
  return { handled, status: res.status, headers: res.headers, body: parsed, raw };
}

/** The stored share-link row behind a token. */
const link = (token) => db.__tables.presentation_share_links.find((r) => r.token === token);
...
async function guestSession(token, { email = 'guest@example.com', name = 'Gwen' } = {}) {
  ...
  const verificationToken = guests().find((g) => g.email === email).verification_token;
  const verified = await call('GET', `/api/share/${token}/guest/verify/${verificationToken}`);
  assert.equal(verified.status, 302, 'the verification link redirects back to the deck');
  return {
...
test('the validation response carries no deck content and no link secrets', async () => {
  ...
  assert.equal(res.body.requiresPassword, true, 'the client is told a password is needed');
  assert.deepEqual(
...
test('a deck id is not a token, so an unshared deck stays unreachable', async () => {
  ...
  for (const deckId of DECKS) {
    ...
    assert.equal(res.status, 404, `${deckId} is not addressable as a share token`);
...
test('a revoked token returns the revocation notice for its own deck only', async () => {
  ...
  assert.equal(res.body.presentationId, undefined, 'the deck id is not handed back');
});
...
test('the right password opens the link; the response carries no hash', async () => {
  ...
  assert.deepEqual(Object.keys(res.body).sort(), ['permission', 'presentationId', 'token']);
...
test('a missing or wrong password buys nothing and costs the link nothing', async () => {
  ...
  const missing = await call('POST', '/api/share/tok-password/verify', { body: {} });
  ...
  assert.equal(wrong.body.presentationId, undefined, 'a failed attempt names no deck');
...
test('a use-limited link stops working once it is spent', async () => {
  ...
  for (const use of [1, 2]) {
    const res = await call('POST', '/api/share/tok-limited/verify', { body: {} });
    assert.equal(res.status, 200, `use ${use} of 2 is allowed`);
    assert.equal(link('tok-limited').use_count, use);
  }

  const spent = await call('POST', '/api/share/tok-limited/verify', { body: {} });
  assert.equal(spent.status, 410);
  assert.equal(spent.body.error, 'max_uses_exceeded');
  assert.equal(link('tok-limited').use_count, 2, 'a refused use is not counted either');
...
test('a guest on an open comment link verifies into a session', async () => {
  ...
  assert.doesNotMatch(cookie, /Secure/, 'plain HTTP in the test, so no Secure flag');
...
test('an invite-only link refuses someone who was never invited', async () => {
  ...
  assert.deepEqual(guests(), [], 'no verification token is minted for a stranger');
});
...
test('an invited guest passes the same invite-only link', async () => {
  ...
  assert.equal(res.status, 200);
  assert.equal(
    typeof guests().find((g) => g.email === 'invited@example.com').verification_token,
    'string'
  );
});
...
test('a guest request without a usable email is a 400', async () => {
  ...
  for (const body of [{}, { email: 'not-an-email' }, { email: '   ' }]) {
    const res = await call('POST', '/api/share/tok-open/guest/request', { body });
...
test('an unknown verification token redirects with an error and sets no cookie', async () => {
  ...
  assert.equal(res.headers.Location, `http://${HOST}/s/tok-open?guest_error=invalid_token`);
