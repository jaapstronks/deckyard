...
test('logo-wall: a deck stored in the legacy numbered form is inline-clickable after the fold', () => {
  ...
  assert.match(
    html,
    /logo-wall-placeholder is-empty[^>]*data-inline-photo="0"/s,
  );
  assert.match(
    html,
    /logo-wall-item[^>]*data-inline-item="logos"[^>]*data-inline-item-index="0"/s,
  );
});
...
test('logo-wall: an empty wall gets a clickable placeholder cell in edit mode only', () => {
  ...
  for (const ctx of [undefined, {}, { mode: 'present' }, { mode: 'thumb' }]) {
    ...
    assert.ok(
      !/data-inline-photo/.test(html),
      `no clickable cell in mode ${ctx?.mode}`,
    );
...
test('logo-wall: a filled logo carries data-inline-photo + item index', () => {
  ...
  assert.match(
    html,
    /logo-wall-placeholder is-empty[^>]*data-inline-photo="1"/s,
  );
...
test('quote: empty primary portrait slot is clickable in edit mode only', () => {
  ...
  for (const ctx of [undefined, {}, { mode: 'present' }]) {
    ...
    assert.ok(
      !html.includes('quote-portrait'),
      `no portrait slot in mode ${ctx?.mode}`,
    );
  }
});
...
test('every empty placeholder carries the shared icon + label inner', () => {
  for (const type of LABELLED_TYPES) {
    const html = SLIDE_TYPES[type].renderHtml(
      { images: [{}] },
      {},
      { mode: 'edit' },
    );
...
test('the placeholder label follows the deck language', () => {
  for (const type of LABELLED_TYPES) {
    const nl = SLIDE_TYPES[type].renderHtml(
      { images: [{}] },
      {},
      { lang: 'nl' },
    );
    const en = SLIDE_TYPES[type].renderHtml(
      { images: [{}] },
      {},
      { lang: 'en-GB' },
    );
...
test('gallery keeps its slot numbering, now localised', () => {
  ...
  const labels = [...nl.matchAll(/image-placeholder-text">([^<]*)/g)].map(
    (m) => m[1],
  );
...
test('placeholders stay decorative for screen readers', () => {
  ...
  for (const type of LABELLED_TYPES) {
    ...
    const boxes =
      html.match(/<div class="[^"]*placeholder[^"]* is-empty"[^>]*>/g) || [];
    assert.ok(boxes.length > 0, `${type}: expected an empty placeholder box`);
    for (const box of boxes) {
      assert.ok(
        box.includes('aria-hidden="true"'),
        `${type}: placeholder must be aria-hidden: ${box}`,
      );
    }
  }
});
...
test('every slide type with an image slot renders the shared placeholder box', () => {
  for (const [type, content] of ALL_PLACEHOLDER_TYPES) {
    ...
    assert.ok(
      html.includes('image-placeholder-icon'),
      `${type}: expected the shared glyph`,
    );
  }
});
...
test('each type keeps its own modifier class for sizing and colour', () => {
  ...
  for (const [type, content] of ALL_PLACEHOLDER_TYPES) {
    ...
    assert.ok(
      html.includes(modifier),
      `${type}: lost its ${modifier} modifier`,
    );
  }
});
...
test('small slots are compact and carry no label', () => {
  ...
  for (const type of ['quote-slide', 'team-cards-slide']) {
    ...
    assert.ok(
      html.includes('is-compact'),
      `${type}: expected the compact modifier`,
    );
    assert.ok(
...
test('the logo-wall label is localised, not hardcoded', () => {
  const def = SLIDE_TYPES['logo-wall-slide'];
  assert.match(
    def.renderHtml({}, {}, { mode: 'edit', lang: 'nl' }),
    /image-placeholder-text">Logo/,
  );
  assert.match(
    def.renderHtml({}, {}, { mode: 'edit', lang: 'en-GB' }),
    /image-placeholder-text">Logo/,
  );
});
...
test('the placeholder glyph is defined in exactly one place', () => {
  ...
  assert.equal(
    boxes.length,
    2,
    'one glyph per empty slot, from the shared helper',
  );
});
