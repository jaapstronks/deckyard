...
test('og:image is read in either attribute order', () => {
  assert.equal(
    parseOgImage('<meta property="og:image" content="https://x.b-cdn.net/a.jpg"/>'),
    'https://x.b-cdn.net/a.jpg'
  );
  assert.equal(
    parseOgImage('<meta content="https://x.b-cdn.net/a.jpg" property="og:image">'),
    'https://x.b-cdn.net/a.jpg'
  );
...
test('the pull zone is taken from a play page that names our video', () => {
  const html = playPage(`https://vz-3c68c0d7-c44.b-cdn.net/${VIDEO_ID}/thumbnail.jpg`);
  assert.equal(bunnyPullZoneFromPlayPage(html, VIDEO_ID), 'vz-3c68c0d7-c44.b-cdn.net');
});

test('a play page pointing somewhere else is refused', () => {
  // Not a Bunny CDN host.
  assert.equal(
    bunnyPullZoneFromPlayPage(playPage(`https://evil.example/${VIDEO_ID}/thumbnail.jpg`), VIDEO_ID),
    null
  );
  // Right host, but a different video than we asked for.
  assert.equal(
    bunnyPullZoneFromPlayPage(
      playPage('https://vz-3c68c0d7-c44.b-cdn.net/00000000-0000-4000-8000-000000000000/thumbnail.jpg'),
      VIDEO_ID
    ),
    null
  );
  ...
  assert.equal(
    bunnyPullZoneFromPlayPage(playPage(`http://vz-x.b-cdn.net/${VIDEO_ID}/thumbnail.jpg`), VIDEO_ID),
    null
  );
...
test('YouTube and Vimeo stills need no referer', async () => {
  const yt = await resolveVideoThumbnailRequest('https://youtu.be/dQw4w9WgXcQ', '366590');
  ...
  const vm = await resolveVideoThumbnailRequest('https://vimeo.com/123456789', '366590');
...
test('a configured pull zone short-circuits discovery, and Bunny gets a referer', async () => {
  ...
  try {
    ...
    assert.equal(req.url, `https://vz-configured.b-cdn.net/${VIDEO_ID}/thumbnail.jpg`);
...
test('an unrecognised source resolves to no thumbnail request', async () => {
  ...
  assert.equal(await resolveVideoThumbnailRequest('not-a-video', '366590'), null);
});
