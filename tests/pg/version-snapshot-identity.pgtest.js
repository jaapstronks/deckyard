 ...
 *  - **Restoring.** Restore is `updatePresentation(scope, id, snapshot, …)`,
...
pgDescribe('version snapshots are identity-clean (real PostgreSQL)', () => {
  ...
  it('snapshotting does not strip the live deck', async () => {
    ...
    assert.equal((await getPresentation(scope, DECK_ID)).ownerEmail, ALICE);
  });
  ...
  it('restoring an identity-bearing legacy snapshot does not move ownership back', async () => {
    ...
    assert.equal((await getPresentation(scope, DECK_ID)).ownerEmail, BOB);

    // Restore, exactly as routes/api/presentations/restore.js does it.
    const version = await getPresentationVersion(scope, DECK_ID, versionId);
    assert.equal(version.presentation.ownerEmail, ALICE, 'the old bag really does carry Alice');

    const before = await getPresentation(scope, DECK_ID);
    await updatePresentation(scope, DECK_ID, version.presentation, {
    ...
    const after = await getPresentation(scope, DECK_ID);
  ...
  it('restoring a stripped snapshot behaves identically', async () => {
    ...
    const version = await getPresentationVersion(scope, DECK_ID, created.id);
    const before = await getPresentation(scope, DECK_ID);
    await updatePresentation(scope, DECK_ID, version.presentation, {
      expectedRevision: before.revision,
      actorEmail: BOB,
