...
pgDescribe('tags storage (real PostgreSQL, via facade)', () => {
  ...
  it('reads back empty on a fresh store instead of throwing', async () => {
    ...
    assert.deepStrictEqual(await getTagsForPresentation(scope, missing), []);
    const map = await getTagsForPresentations(scope, [p1, p2]);
  ...
  it('shares a tag id across presentations by name', async () => {
    ...
    const got1 = await getTagsForPresentation(scope, p1);
    const got2 = await getTagsForPresentation(scope, p2);
