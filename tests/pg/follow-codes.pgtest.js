...
pgDescribe('follow-code storage (real PostgreSQL)', () => {
  ...
  it('mints a five-character code and resolves it back to its follow URL', async () => {
    ...
    const code = await createFollowCode(null, followUrl);
    ...
    assert.equal(await resolveFollowCode(null, code), followUrl);
  });

  it('resolves case-insensitively and misses on an unknown code', async () => {
    const code = await createFollowCode(null, '/follow/deck-42?lang=en-GB');
    assert.equal(await resolveFollowCode(null, code.toLowerCase()), '/follow/deck-42?lang=en-GB');
    assert.equal(await resolveFollowCode(null, 'ZZZZZ'), null);
  });

  it('mints distinct codes for the two languages of one deck', async () => {
    const nl = await createFollowCode(null, '/follow/deck-42?lang=nl');
    const en = await createFollowCode(null, '/follow/deck-42?lang=en-GB');
    assert.notEqual(nl, en);
    assert.equal(await resolveFollowCode(null, nl), '/follow/deck-42?lang=nl');
    assert.equal(await resolveFollowCode(null, en), '/follow/deck-42?lang=en-GB');
  });

  it('refuses to resolve an expired code and deletes it on the way out', async () => {
    const code = await createFollowCode(null, '/follow/deck-42?lang=nl');
    await db
    ...
    assert.equal(await resolveFollowCode(null, code), null);
  ...
  it('sweeps expired codes and leaves live ones alone', async () => {
    const live = await createFollowCode(null, '/follow/live?lang=nl');
    const stale = await createFollowCode(null, '/follow/stale?lang=nl');
    await db
      .updateTable('follow_codes')
    ...
    assert.equal(await cleanupExpiredCodes(), 1);
    ...
    assert.equal(await cleanupExpiredCodes(), 0);
  });
});
