...
pgDescribe(
  'collaborator authz resolution + identity resolver (real PostgreSQL)',
  () => {
    ...
    it('returns null for someone who is not a collaborator', async () => {
      assert.equal(
        await getCollaboratorPermission(PID, 'stranger@example.com'),
        null,
      );
    });
    ...
    it('resolves a row that carries a foreign organization stamp', async () => {
      ...
      await db
        .insertInto('organizations')
        .values({ id: otherOrg, name: 'Third', slug: 'third' })
        .execute();
