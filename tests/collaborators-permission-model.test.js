...
const ACTORS = {
  ...
  newcomer: { email: 'newcomer@example.com', name: 'Nils', organizationId: ORG },
...
async function seed() {
  db = createFakeDb({
    ...
    presentation_collaborators: [
      collaboratorRow({ id: 'c-admin', user_email: ACTORS.admin.email, permission: 'admin' }),
      collaboratorRow({ id: 'c-editor', user_email: ACTORS.editor.email, permission: 'edit' }),
      collaboratorRow({ id: 'c-viewer', user_email: ACTORS.viewer.email, permission: 'view' }),
      collaboratorRow({
        id: 'c-revoked',
...
const rowsFor = (deckId) =>
  db.__tables.presentation_collaborators.filter((r) => r.presentation_id === deckId);
...
test('shared-with-me lists the decks shared with the caller, newest invite first', async () => {
  await seed();
  const res = await call('GET', '/api/presentations/shared-with-me', { as: ACTORS.viewer });
...
test('a revoked invite stops appearing in shared-with-me', async () => {
  await seed();
  const res = await call('GET', '/api/presentations/shared-with-me', { as: ACTORS.revoked });
...
test('someone nobody shared with sees an empty list, not everyone else’s decks', async () => {
  await seed();
  const res = await call('GET', '/api/presentations/shared-with-me', { as: ACTORS.stranger });
...
test('the owner adds a collaborator, and the invite is recorded once', async () => {
  ...
  assert.equal(row.organization_id, ORG, 'the row lands in the acting organization');
  assert.equal(row.invited_by, ACTORS.owner.email);
  assert.equal(row.user_id, 'user-newcomer', 'the stable user id is written beside the email');
});
...
test('adding a collaborator notifies them and lands in the activity feed', async () => {
  ...
  const notification = notifications().find((n) => n.user_email === ACTORS.newcomer.email);
...
test('a collaborator holding admin may add collaborators', async () => {
  ...
  assert.equal(res.status, 201);
  assert.equal(rowFor('deck-owned', ACTORS.newcomer.email).invited_by, ACTORS.admin.email);
});
...
test('nobody else may add a collaborator', async () => {
  for (const [key, why] of REFUSED) {
    ...
    assert.equal(rowFor('deck-owned', ACTORS.newcomer.email), undefined, 'no row is written');
...
test('a deck in another organization is absent, not forbidden', async () => {
  await seed();
  const res = await call('POST', '/api/presentations/deck-foreign/collaborators', {
    as: ACTORS.owner,
    body: { userEmail: ACTORS.newcomer.email, permission: 'edit' },
  });

  assert.equal(res.status, 404, 'the deck never reaches the authorization check');
...
test('an unknown deck is a 404', async () => {
  await seed();
  const res = await call('POST', '/api/presentations/deck-does-not-exist/collaborators', {
    as: ACTORS.owner,
    body: { userEmail: ACTORS.newcomer.email, permission: 'view' },
  });
...
test('an invalid permission is refused before anything is written', async () => {
  ...
  for (const permission of ['owner', 'ADMIN', '', null, undefined]) {
    ...
    assert.equal(res.status, 400, `${JSON.stringify(permission)} is not a permission`);
  }
...
test('a body without a usable address is a 400', async () => {
  ...
  assert.equal(rowsFor('deck-owned').length, 4, 'the seeded rows are all there is');
});
...
test('a batch larger than twenty is refused whole', async () => {
  ...
  const res = await call('POST', '/api/presentations/deck-owned/collaborators', {
    ...
    body: {
      permission: 'view',
      userEmails: Array.from({ length: 21 }, (_, i) => `person${i}@example.com`),
    },
  });

  assert.equal(res.status, 400);
  assert.equal(rowsFor('deck-owned').length, 4, 'not one of the twenty-one is written');
});
...
test('a batch reports per address and does not let one failure sink the rest', async () => {
  ...
  const res = await call('POST', '/api/presentations/deck-owned/collaborators', {
    ...
    body: {
      permission: 'view',
      userEmails: [ACTORS.newcomer.email, 'ghost@example.com', ACTORS.viewer.email],
    },
  });
...
test('the collaborator list of a deck in another organization is a 404', async () => {
  await seed();
  const res = await call('GET', '/api/presentations/deck-foreign/collaborators', {
    as: ACTORS.owner,
  });
...
test('an url-encoded address in the path reaches the right row', async () => {
  ...
  assert.equal(res.status, 200);
  assert.equal(typeof rowFor('deck-owned', ACTORS.editor.email).revoked_at, 'string');
});
...
test('nobody but the owner or an admin may revoke', async () => {
  for (const [key, why] of REFUSED) {
    ...
    assert.equal(res.status, 401, `${key ?? 'anonymous'}: ${why}`);
    assert.equal(rowFor('deck-owned', ACTORS.admin.email).revoked_at, null, 'the invite stands');
  }
});
...
test('an admin collaborator may revoke', async () => {
  ...
  assert.equal(res.status, 200);
  assert.equal(rowFor('deck-owned', ACTORS.editor.email).revoked_by, ACTORS.admin.email);
});
...
test('the handler declines everything outside its five endpoints', async () => {
  ...
  const declined = [
    ...
    ['POST', `/api/presentations/deck-owned/collaborators/${ACTORS.viewer.email}`],
    ['GET', `/api/presentations/deck-owned/collaborators/${ACTORS.viewer.email}`],
