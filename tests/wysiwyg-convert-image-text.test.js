...
test('lossy confirm stays quiet for defaults in both directions', () => {
  ...
  assert.deepEqual(
    getConversionLossyKeys(contentSlide(), 'image-text-slide'),
    [],
  );
  assert.deepEqual(
    getConversionLossyKeys(
      contentSlide({ layout: 'two-column' }),
      'image-text-slide',
    ),
    [],
  );
  ...
  assert.deepEqual(
    getConversionLossyKeys(
      imageTextSlide({ focusX: 30, focusY: 60 }),
      'content-slide',
    ),
    [],
  );
...
test('removeMedia selector matches the rendered empty placeholder only', () => {
  ...
  const empty = SLIDE_TYPES['image-text-slide'].renderHtml({
    title: 'T',
    body: 'b',
  });
...
test('addMedia anchor exists in the rendered content slide', () => {
  const add = INLINE_DESCRIPTORS['content-slide'].convert.addMedia;
  const html = SLIDE_TYPES['content-slide'].renderHtml({
    title: 'T',
    body: 'b',
  });
