 ...
 * multi-workspace (A1 org-scoping, deelbesluit 4).
 *
 ...
 * take an organization from. That is a valid answer only in single-workspace
 * mode; once an instance holds several workspaces, serving one workspace's
 * presentations under the instance-global `/feed/*.xml` URL (and advertising it
 * with `<link rel="alternate">`) would leak that workspace across the instance.
 * So under `MULTI_WORKSPACE_ENABLED` the feed routes 404 and the autodiscovery
 * links are omitted.
 *
 ...
 * The single-workspace mirror — same setup, gate transparent — is in
 * feed-single-workspace.test.js.
 *
 ...
 *
 * Run with: node --test tests/feed-multi-workspace-gate.test.js
 */
...
delete process.env.RSS_FEED_ENABLED; // default: RSS on, so only the multi-workspace gate can 404
...
// here can only be the multi-workspace gate, never a missing/disabled org.
function installDb() {
...
for (const [pathname, format] of [
  ...
  test(`handleFeed 404s ${format} under multi-workspace, before touching the database`, async () => {
    const db = installDb();
    const res = mockRes();
    ...
    assert.equal(res.statusCode, 404, 'the feed 404s under multi-workspace');
    assert.ok(
...
test('injectFeedDiscovery omits the autodiscovery links under multi-workspace', async () => {
  const db = installDb();
  const html = await injectFeedDiscovery('<head></head>', null);

  assert.ok(
    !html.includes('rel="alternate"'),
    'no feed <link> is advertised under multi-workspace'
  );
  assert.ok(
    !touchedTables(db).includes('organizations'),
