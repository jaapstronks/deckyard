...
import { writeUploadedFile, replaceUploadFromDataUrl } from '../server/storage/uploads.js';
...
test('upload validation throws 400 ValidationError', async () => {
  ...
  await assert.rejects(
    () => replaceUploadFromDataUrl(repoRoot, '/etc/passwd', 'data:image/png;base64,AAAA'),
    isValidationError
  );
  await assert.rejects(
    () => replaceUploadFromDataUrl(repoRoot, '/uploads/x.zip', 'data:image/png;base64,AAAA'),
    isValidationError
  );
});
...
test('replacing a nonexistent upload throws 404 NotFoundError', async () => {
  ...
  await assert.rejects(
    () => replaceUploadFromDataUrl(scratchRoot, '/uploads/nope.png', 'data:image/png;base64,AAAA'),
...
test('collab authorization failures keep their statuses', async () => {
  await assert.rejects(
    () => authorizeDocument({ repoRoot, documentName: 'not-a-collab-doc', user: { email: 'a@b.c' } }),
    (err) => err instanceof NotFoundError && err.statusCode === 404
  );
  await assert.rejects(
    () => authorizeDocument({ repoRoot, documentName: 'presentation:some-id', user: {} }),
