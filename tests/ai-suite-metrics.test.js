...
import {
  ...
  aggregateScores,
  overallScore,
} from '../test-suite/eval/report.js';
      ...
      rows: [
        {
          ...
          blocks: [
            {
              title: 'Split transitions',
              body: 'The two transitions rarely meet',
            },
          ],
        },
      ],
...
test('numberFidelity flags figures that are absent from the source', () => {
  const deck = {
    slides: [
      {
        type: 'content-slide',
        content: { title: 'Results', body: 'Revenue was 9300 million.' },
      },
      {
        type: 'content-slide',
        content: { title: 'Made up', body: 'Margin hit 87 percent.' },
      },
    ],
  };
  ...
  assert.ok(
    !result.unsupported.includes('9300'),
...
test('numberFidelity ignores years and small integers', () => {
  const deck = {
    slides: [
      { type: 'content-slide', content: { body: 'In 2031 we had 3 pillars.' } },
    ],
  };
...
test('numberFidelity matches across EU and US number formatting', () => {
  const deck = {
    slides: [
      { type: 'content-slide', content: { body: 'Total of 1.234,5 million.' } },
    ],
  };
  ...
  assert.deepEqual(
    result.unsupported,
    [],
    'same value written both ways still matches',
  );
});
...
test('computeDeltas only calls a move significant past the threshold', () => {
  ...
  const byDimension = Object.fromEntries(
    deltas.map((d) => [d.dimension, d.direction]),
  );
...
test('costOf prices cached reads far below fresh input', () => {
  ...
  const cached = costOf(
    { ...empty, cacheReadTokens: 1_000_000 },
    'claude-opus-4-8',
  );
  assert.equal(fresh, 5);
  assert.ok(
    cached < fresh / 5,
    'cache reads are an order of magnitude cheaper',
  );
});
...
test('CostTracker keeps categories and models separate, and totals them', () => {
  ...
  assert.equal(
    summary.totalUsd,
    Math.round((judge.usd + generation.usd) * 10000) / 10000,
  );
});
...
test('deckMetrics measures type monotony and divider share', () => {
  const slide = (type) => ({
    type,
    content: { title: 'x', body: 'some words here' },
  });
  const deck = {
    slides: [
      slide('title-slide'),
  ...
  assert.equal(
    repetition.consecutiveRepeats,
    2,
    'three in a row is two repeats',
  );
