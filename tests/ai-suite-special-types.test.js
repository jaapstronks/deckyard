...
const deckOf = (...types) => ({
  slides: types.map((type) => ({ type, content: {} })),
});
...
test('a missed specialised type is reported with the reason it was expected', () => {
  const r = specialTypeUsage(
    deckOf('title-slide', 'list-slide', 'list-slide', 'kpi-metrics-slide'),
    expected,
  );
  assert.equal(r.recall, 0.33);
  assert.deepEqual(
    r.missing.map((m) => m.type),
    ['process-slide', 'timeline-slide'],
  );
  assert.equal(
    r.missing[0].because,
    'five ordered steps',
    'the reason travels with the miss',
  );
});
...
test('the generic types the content fell back to are named', () => {
  ...
  const r = specialTypeUsage(
    deckOf('title-slide', 'list-slide', 'content-slide'),
    expected,
  );
...
test('a declared acceptable alternative counts as a hit', () => {
  ...
  const withAlt = [
    {
      ...
      acceptable: ['table-slide'],
      because: 'before/after pilot figures',
    },
  ];
  ...
  assert.deepEqual(
    r.found,
    ['table-slide'],
    'the alternative actually used is reported',
  );
});
...
test('an alternative that was not declared is still a miss', () => {
  ...
  const r = specialTypeUsage(
    deckOf('title-slide', 'text-blocks-slide'),
    strict,
  );
