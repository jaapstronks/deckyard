 ...
 * actually seeded from is the slide factory (`newSlide`), which reads
 ...
 *
 * This test pins that behaviour so a refactor of the factory (or a type that
...
/**
 * The factory injects a fresh `pollId` for poll-slide (a runtime id, not
...
test('a new slide is seeded from the deck-language defaults', () => {
  for (const type of BILINGUAL_TYPES) {
    ...
    const nl = newSlide({ type, slideTypes: SLIDE_TYPES, lang: 'nl' });
    const en = newSlide({ type, slideTypes: SLIDE_TYPES, lang: 'en-GB' });
    assert.deepEqual(
...
test('an unknown or missing deck language falls back to the English defaults', () => {
  // The factory only honours 'nl' / 'en-GB'; anything else (or nothing) takes
  // the plain `defaults`. This is the graceful path for a Tier-2 deck language.
  const type = BILINGUAL_TYPES[0];
  const def = SLIDE_TYPES[type];
  const unknown = newSlide({ type, slideTypes: SLIDE_TYPES, lang: 'de' });
  const none = newSlide({ type, slideTypes: SLIDE_TYPES });
  assert.deepEqual(
