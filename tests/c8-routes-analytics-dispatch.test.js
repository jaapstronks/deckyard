...
test('analytics(index): dashboard + GDPR routes resolve in order', () => {
  ...
  named(INDEX_ROUTES, 'GET', '/api/analytics/presentations', 'handlePresentationsList');
...
test('analytics(index): per-presentation metrics routes resolve with the id capture', () => {
  named(INDEX_ROUTES, 'GET', '/api/presentations/p-1/analytics', 'handleOverview');
  named(INDEX_ROUTES, 'GET', '/api/presentations/p-1/analytics/slides', 'handleSlides');
  named(INDEX_ROUTES, 'GET', '/api/presentations/p-1/analytics/heatmap', 'handleHeatmap');
  named(INDEX_ROUTES, 'GET', '/api/presentations/p-1/analytics/journey', 'handleJourney');
  named(INDEX_ROUTES, 'GET', '/api/presentations/p-1/analytics/sessions', 'handleSessions');
  named(INDEX_ROUTES, 'GET', '/api/presentations/p-1/analytics/realtime', 'handleRealtime');

  const overview = select(INDEX_ROUTES, 'GET', '/api/presentations/p-1/analytics');
  assert.deepEqual(overview.pattern.exec('/api/presentations/p-1/analytics').slice(1), ['p-1']);
});

test('analytics(index): report CRUD splits by method on the same pattern, in order', () => {
  named(INDEX_ROUTES, 'GET', '/api/presentations/p-1/analytics/reports', 'handleListReports');
  named(INDEX_ROUTES, 'POST', '/api/presentations/p-1/analytics/reports', 'handleCreateReport');
  named(INDEX_ROUTES, 'GET', '/api/presentations/p-1/analytics/reports/r-1', 'handleGetReport');
  named(INDEX_ROUTES, 'PATCH', '/api/presentations/p-1/analytics/reports/r-1', 'handleUpdateReport');
  named(INDEX_ROUTES, 'DELETE', '/api/presentations/p-1/analytics/reports/r-1', 'handleDeleteReport');
  named(
    INDEX_ROUTES,
    'POST',
  ...
  const item = select(INDEX_ROUTES, 'GET', '/api/presentations/p-1/analytics/reports/r-1');
  assert.deepEqual(
...
test('analytics(index): overlapping report patterns do not swallow one another', () => {
  ...
  assert.equal(select(INDEX_ROUTES, 'GET', '/api/presentations/p-1/analytics/reports')?.handler.name, 'handleListReports');
  assert.equal(select(INDEX_ROUTES, 'GET', '/api/presentations/p-1/analytics/reports/r-1')?.handler.name, 'handleGetReport');
  // regenerate-token is POST-only; a GET on that path matches nothing.
  assert.equal(select(INDEX_ROUTES, 'GET', '/api/presentations/p-1/analytics/reports/r-1/regenerate-token'), null);
});
...
test('analytics(index): a wrong method on a report path matches no route (falls through, no 405)', () => {
  ...
  assert.equal(select(INDEX_ROUTES, 'DELETE', '/api/presentations/p-1/analytics/reports'), null);
  assert.equal(select(INDEX_ROUTES, 'PUT', '/api/presentations/p-1/analytics/reports/r-1'), null);
...
test('analytics-track: the five tracking routes resolve to their named handlers', () => {
  named(TRACK_ROUTES, 'POST', '/api/track/session/start', 'handleTrackSessionStart');
  named(TRACK_ROUTES, 'POST', '/api/track/session/heartbeat', 'handleTrackSessionHeartbeat');
  named(TRACK_ROUTES, 'POST', '/api/track/session/end', 'handleTrackSessionEnd');
  named(TRACK_ROUTES, 'POST', '/api/track/slide/view', 'handleTrackSlideView');
  named(TRACK_ROUTES, 'POST', '/api/track/my-data/erase', 'handleTrackMyDataErase');
});
...
test('analytics-track: a wrong method falls through without touching a handler', async () => {
  ...
  for (const path of [
    ...
    assert.equal(select(TRACK_ROUTES, 'GET', path), null, `GET ${path} matches no route`);
    assert.equal(await handleAnalyticsTrack(ctx('GET', path)), false, `GET ${path} → false`);
  }
  assert.equal(await handleAnalyticsTrack(ctx('POST', '/api/track/unknown')), false);
});
...
test('analytics-public: the report-by-token route resolves with the token capture', () => {
  named(PUBLIC_ROUTES, 'GET', '/api/analytics/reports/abc123', 'handlePublicReport');
  const route = select(PUBLIC_ROUTES, 'GET', '/api/analytics/reports/abc123');
  assert.deepEqual(route.pattern.exec('/api/analytics/reports/abc123').slice(1), ['abc123']);
});

test('analytics-public: a wrong method or unknown path falls through', async () => {
  assert.equal(select(PUBLIC_ROUTES, 'POST', '/api/analytics/reports/abc123'), null);
  assert.equal(await handleAnalyticsReportPublic(ctx('POST', '/api/analytics/reports/abc123')), false);
  assert.equal(await handleAnalyticsReportPublic(ctx('GET', '/api/analytics/other')), false);
});
