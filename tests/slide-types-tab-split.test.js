...
const { getCategories, CATEGORY_LABELS } =
  await import('../client/views/settings/tabs/slide-types-tab/categories.js');
const { createCurationThumbnail, createVideoMockup } =
  await import('../client/views/settings/tabs/slide-types-tab/curation-thumbnails.js');
const { openTypePreview } =
  await import('../client/views/settings/tabs/slide-types-tab/type-preview-modal.js');
...
test('categories: every category key resolves to a label and holds types', () => {
  ...
  for (const cat of categories) {
    assert.equal(
      typeof CATEGORY_LABELS[cat.key],
      'function',
      `label for ${cat.key}`,
    );
    assert.equal(typeof CATEGORY_LABELS[cat.key](), 'string');
    assert.ok(
      Array.isArray(cat.types) && cat.types.length > 0,
      `types for ${cat.key}`,
    );
  }
});
...
test('thumbnails: video slide routes to the play-button mockup', () => {
  ...
  const viaThumbnail = createCurationThumbnail(
    'video-slide',
...
test('thumbnails: a normal type never throws, always returns an element', () => {
  const el = createCurationThumbnail(
    'content-slide',
    'slide-type-curation-thumb',
    null,
  );
...
test('preview modal: opens a backdrop and closes on Escape', () => {
  ...
  openTypePreview('content-slide', allTypesList, {
    slideTypeMeta: {
      'content-slide': { label: 'Content' },
      'quote-slide': { label: 'Quote' },
    },
  ...
  assert.equal(
    document.querySelector('.slide-type-preview-backdrop'),
    null,
    'closes on Escape',
  );
