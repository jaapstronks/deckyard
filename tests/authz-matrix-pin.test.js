...
describe('canReadPresentation — private deck', () => {
  it('owner and creator can read', () => {
    ...
    assert.equal(canReadPresentation({ user: CREATOR, pres: privateDeck }), true);
  });
  it('a collaborator with any permission can read', () => {
    for (const p of [VIEW, COMMENT, EDIT, ADMIN_PERM]) {
      assert.equal(
        canReadPresentation({ user: OTHER, pres: privateDeck, collaboratorPermission: p }),
        true,
  ...
  it('an unrelated user cannot read', () => {
    assert.equal(canReadPresentation({ user: OTHER, pres: privateDeck }), false);
  });
  ...
  it('the unrestricted operator can always read', () => {
    assert.equal(canReadPresentation({ user: OPERATOR, pres: privateDeck }), true);
  });
});
...
describe('canWritePresentation — private deck', () => {
  it('owner and creator can write', () => {
    assert.equal(canWritePresentation({ user: OWNER, pres: privateDeck }), true);
    assert.equal(canWritePresentation({ user: CREATOR, pres: privateDeck }), true);
  });
  it('collaborator: edit/admin can write, view/comment cannot', () => {
    assert.equal(
      canWritePresentation({ user: OTHER, pres: privateDeck, collaboratorPermission: EDIT }),
      true
    );
    assert.equal(
      canWritePresentation({ user: OTHER, pres: privateDeck, collaboratorPermission: ADMIN_PERM }),
      true
    );
    assert.equal(
      canWritePresentation({ user: OTHER, pres: privateDeck, collaboratorPermission: COMMENT }),
      false
    );
    assert.equal(
      canWritePresentation({ user: OTHER, pres: privateDeck, collaboratorPermission: VIEW }),
      false
    );
  });
  it('an unrelated user and an actor with no email cannot write', () => {
    assert.equal(canWritePresentation({ user: OTHER, pres: privateDeck }), false);
    assert.equal(canWritePresentation({ user: ANON, pres: privateDeck }), false);
  });
});
...
describe('canWritePresentation — view-only and workspace', () => {
  ...
  it('a would-be collaborator is blocked by view-only before permission is consulted', () => {
    assert.equal(
      canWritePresentation({ user: OTHER, pres: viewOnlyPrivate, collaboratorPermission: EDIT }),
      false
    );
  });
...
describe('canDeletePresentation', () => {
  ...
  it('the unrestricted operator can delete', () => {
    assert.equal(canDeletePresentation({ user: OPERATOR, pres: privateDeck }), true);
  });
});
...
describe('canForceLockRelease (slide locks)', () => {
  it('owner and creator can force-release; others cannot', () => {
    ...
    assert.equal(canForceLockRelease({ user: CREATOR, pres: privateDeck }), true);
    assert.equal(canForceLockRelease({ user: OTHER, pres: privateDeck }), false);
    assert.equal(canForceLockRelease({ user: ANON, pres: privateDeck }), false);
  });
  it('the unrestricted operator can force-release', () => {
    assert.equal(canForceLockRelease({ user: OPERATOR, pres: privateDeck }), true);
  });
});

describe('canManageCollaborators', () => {
  it('owner and creator can manage', () => {
    assert.equal(canManageCollaborators({ user: OWNER, pres: privateDeck }), true);
    assert.equal(canManageCollaborators({ user: CREATOR, pres: privateDeck }), true);
  });
  it('only an admin-level collaborator can manage; edit and below cannot', () => {
    assert.equal(
      canManageCollaborators({ user: OTHER, pres: privateDeck, collaboratorPermission: ADMIN_PERM }),
      true
    );
    assert.equal(
      canManageCollaborators({ user: OTHER, pres: privateDeck, collaboratorPermission: EDIT }),
      false
    );
  });
  it('an unrelated user cannot manage', () => {
    assert.equal(canManageCollaborators({ user: OTHER, pres: privateDeck }), false);
  });
});

describe('canCommentOnPresentation', () => {
  it('owner and creator can comment', () => {
    assert.equal(canCommentOnPresentation({ user: OWNER, pres: privateDeck }), true);
    assert.equal(canCommentOnPresentation({ user: CREATOR, pres: privateDeck }), true);
  });
  it('collaborator: comment/edit/admin can comment, view cannot', () => {
    assert.equal(
      canCommentOnPresentation({ user: OTHER, pres: privateDeck, collaboratorPermission: COMMENT }),
      true
    );
    assert.equal(
      canCommentOnPresentation({ user: OTHER, pres: privateDeck, collaboratorPermission: EDIT }),
      true
    );
    assert.equal(
      canCommentOnPresentation({ user: OTHER, pres: privateDeck, collaboratorPermission: ADMIN_PERM }),
      true
    );
    assert.equal(
      canCommentOnPresentation({ user: OTHER, pres: privateDeck, collaboratorPermission: VIEW }),
      false
    );
  });
...
describe('isPresentationAuthor', () => {
  it('true for owner and creator, false for everyone else', () => {
    assert.equal(isPresentationAuthor({ user: OWNER, pres: privateDeck }), true);
    assert.equal(isPresentationAuthor({ user: CREATOR, pres: privateDeck }), true);
    assert.equal(
      isPresentationAuthor({ user: OTHER, pres: privateDeck, collaboratorPermission: ADMIN_PERM }),
      false
    );
    assert.equal(isPresentationAuthor({ user: ANON, pres: privateDeck }), false);
  });
});

describe('getEffectivePermission', () => {
  it('owner and creator get edit', () => {
    assert.equal(getEffectivePermission({ user: OWNER, pres: privateDeck }), 'edit');
    assert.equal(getEffectivePermission({ user: CREATOR, pres: privateDeck }), 'edit');
  });
  it('a collaborator falls through to their raw permission verbatim', () => {
    ...
    assert.equal(
      getEffectivePermission({ user: OTHER, pres: privateDeck, collaboratorPermission: EDIT }),
      'edit'
    );
    assert.equal(
      getEffectivePermission({ user: OTHER, pres: privateDeck, collaboratorPermission: COMMENT }),
      'comment'
    );
    assert.equal(
      getEffectivePermission({ user: OTHER, pres: privateDeck, collaboratorPermission: VIEW }),
      'view'
    );
    assert.equal(
      getEffectivePermission({ user: OTHER, pres: privateDeck, collaboratorPermission: ADMIN_PERM }),
      'admin'
    );
  });
...
describe('comment moderation (canResolveComment / canDeleteComment)', () => {
  ...
  it('delete: the comment author can delete their own comment', () => {
    ...
    assert.equal(canDeleteComment({ user: OTHER, pres: privateDeck, comment }), true);
  });
  it('delete: the deck owner can moderate anyone else’s comment', () => {
    const comment = { authorEmail: 'other@example.com' };
    assert.equal(canDeleteComment({ user: OWNER, pres: privateDeck, comment }), true);
  });
  it('delete: an unrelated user cannot delete a comment they did not write', () => {
    const comment = { authorEmail: 'someoneelse@example.com' };
    assert.equal(canDeleteComment({ user: OTHER, pres: privateDeck, comment }), false);
  });
});
