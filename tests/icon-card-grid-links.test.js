...
describe('icon-card-grid per-card links', () => {
  it('renders an external URL as a sanitized new-tab anchor', () => {
    const html = render(
      [{ title: 'Docs', body: 'x', link: 'https://example.com/a?b=1' }],
      {
        mode: 'present',
      },
    );
    assert.match(
      html,
      /<a class="card-link" href="https:\/\/example\.com\/a\?b=1"/,
    );
  ...
  it('renders mailto links', () => {
    const html = render(
      [{ title: 'Mail', body: 'x', link: 'mailto:hi@example.com' }],
      {
        mode: 'follow',
      },
    );
  ...
  it('renders #slide:<id> as a stable in-deck nav anchor in presenter mode', () => {
    const html = render([{ title: 'Go', body: 'x', link: '#slide:abc-123' }], {
      mode: 'present',
    });
    assert.match(
      html,
      /<a class="card-link" data-card-nav-id="abc-123" href="#"/,
    );
  ...
  it('renders #N as an in-deck nav anchor in presenter mode', () => {
    const html = render([{ title: 'Go', body: 'x', link: '#3' }], {
      mode: 'present',
    });
  ...
  it('ignores javascript: and other unsafe/relative schemes', () => {
    for (const link of [
      ...
      const html = render([{ title: 'X', body: 'x', link }], {
        mode: 'present',
      });
  ...
  it('omits links in thumbnail previews', () => {
    const html = render(
      [{ title: 'Docs', body: 'x', link: 'https://example.com' }],
      {
        mode: 'thumb',
      },
    );
  ...
  it('omits links in the inline-edit canvas (mode: edit) so click-to-edit is not blocked', () => {
    for (const link of ['https://example.com', '#3']) {
      const html = render([{ title: 'Docs', body: 'x', link }], {
        mode: 'edit',
      });
