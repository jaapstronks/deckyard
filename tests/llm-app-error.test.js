...
test('LlmError is an AppError with the canonical envelope', () => {
  ...
  assert.equal(err.response, 'raw provider payload that must never leave the server');
  // …but never in the serialized envelope.
  assert.ok(!('response' in body) && !('vendor' in body) && !('context' in body));
});

test('LlmError statuses are unchanged by the AppError conversion', () => {
  assert.equal(new LlmError('x').statusCode, 502);
  assert.equal(LlmError.fromProviderFailure('openai', 500, 'body').statusCode, 502);
  ...
  assert.equal(LlmError.fromProviderFailure('openai', 401, '').retryable, false);
});

test('requireEnv throws a 400 ValidationError for a missing variable', () => {
  assert.throws(
    () => requireEnv('DEFINITELY_NOT_SET_LLM_VAR'),
    (err) => err instanceof ValidationError && err.statusCode === 400 && isAppError(err)
  );
});
...
test('no bare .statusCode assignment left under server/utils/llm/', () => {
  ...
  assert.deepEqual(offenders, [], `LLM modules must throw AppError subclasses: ${offenders.join(', ')}`);
});
