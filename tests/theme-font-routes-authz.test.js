...
test('any authed user can list themes and the curated font catalog', async () => {
  seed();
  const themes = await call(handleThemes, 'GET', '/api/themes', {
    as: ACTORS.member,
  });
  assert.equal(themes.res.statusCode, 200);
  assert.ok(
    Array.isArray(themes.res.body.themes),
    'a themes array is returned',
  );

  const fonts = await call(handleThemes, 'GET', '/api/themes/fonts', {
    as: ACTORS.member,
  });
...
test('a designer can create a custom theme; a plain member gets a 403', async () => {
  ...
  assert.equal(
    ...
    403,
    'theme mutations deny with forbidden',
  );
...
test('listing font families needs a session', async () => {
  ...
  const member = await call(handleFontFamilies, 'GET', '/api/font-families', {
    as: ACTORS.member,
  });
...
test('the Adobe discovery route gates and validates before it reaches the network', async () => {
  ...
  assert.equal(
    missing.res.statusCode,
    400,
    'a missing projectId is a 400 before any fetch',
  );
  ...
  assert.equal(
    malformed.res.statusCode,
    400,
    'a malformed projectId is a 400 before any fetch',
  );
});
...
test('the file-upload variant route is gated before it reaches the media provider', async () => {
  ...
  const denied = await call(
    handleFontFamilies,
    'POST',
    `/api/font-families/${id}/upload-variant`,
    {
      as: ACTORS.member,
