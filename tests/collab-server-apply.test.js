...
/**
 * Fill in the columns an INSERT never names.
 *
 * PostgreSQL answers `RETURNING *` with every column, NULL where nothing was
 * written; the double keeps only the columns the INSERT listed, so
 * `mapPresentationRow` turns the absent ones into own properties holding
 * `undefined`. That difference is invisible almost everywhere, but
 * `onStoreDocument` compares the doc projection against the stored deck with
 * `isDeepStrictEqual` (server/collab/persistence.js) to decide whether a JSON
 * write can be skipped — and a key that exists as `undefined` on one side and
 * not at all on the other makes that comparison false forever, so the
 * store-loop guard this test asserts would never fire.
 */
function fillNullColumns() {
  for (const row of db.__tables.presentations || []) {
    row.published ??= null;
    row.trashed_at ??= null;
    row.trashed_by ??= null;
  }
...
const bilingualPres = () => ({
  ...
  slides: [
    ...
    },
    {
      ...
      type: 'content-slide',
      notes: '',
      content: { title: 'Tekst NL', body: 'body nl' },
    },
  ],
  i18n: {
    active: 'nl',
      ...
      'en-GB': {
        ...
        slides: [
          ...
          },
          {
            ...
            type: 'content-slide',
            notes: '',
            content: { title: 'Text EN', body: 'body en' },
          },
        ],
      },
    },
...
test('applyPresentationToDoc: multi-language text + item + plain diffs', () => {
  ...
  next.i18n.versions['en-GB'].slides[0].content.items.push({
    title: 'Three',
    text: 'third',
  });
  ...
  assert.equal(
    out.i18n.versions['en-GB'].slides[0].content.title,
    'List EN v2',
  );
  assert.equal(
    out.i18n.versions['en-GB'].slides[0].content.items[1].text,
    'second v2',
  );
  assert.equal(
    out.slides[0].content.items[1].text,
    'tweede',
    'NL item text untouched',
  );
  assert.equal(out.slides[0].content.textSize, 'large');
  assert.equal(out.slides[0].content.items[2].title, 'Drie');
  assert.equal(
    out.i18n.versions['en-GB'].slides[0].content.items[2].title,
    'Three',
  );
});
...
test('applyPresentationToDoc: structural changes (add/remove/reorder slides)', () => {
  ...
  next2.i18n.versions['en-GB'].slides = next2.i18n.versions[
    'en-GB'
  ].slides.filter((s) => s.id !== 's1');
...
test('applyPresentationToDoc: warnings for divergence, drops and languages', () => {
  ...
  assert.ok(
    warnings.some((w) =>
      w.includes("plain field 'textSize' differs in version 'en-GB'"),
    ),
    `plain divergence warning, got: ${warnings.join(' | ')}`,
  );
  assert.ok(
    warnings.some((w) =>
      w.includes("slide ghost only exists in version 'en-GB'"),
    ),
  ...
  assert.ok(
    w2.some((w) =>
      w.includes("language version 'en-GB' is not in the incoming deck"),
    ),
  ...
  assert.equal(
    ytitle.get('en-GB'),
    undefined,
    'removed language title cleaned up',
  );
});
...
test('applyPresentationToDoc: never stores i18n.active; slide type change reclassifies', () => {
  ...
  assert.equal(
    out.i18n.versions['en-GB'].slides[1].content.items[0].title,
    'A(en)',
  );
});
...
test('applyPresentationToDoc with base: three-way diff spares concurrent doc state', () => {
  ...
  assert.equal(
    out.slides.find((s) => s.id === 's2').content.body,
    'server body',
  );
  ...
  assert.equal(out.slides[0].content.title, 'Lijstje NL (client)');
  assert.ok(
    out.slides.some((s) => s.id === 'client-slide'),
    'client-added slide survives',
  );
  assert.equal(
    out.slides[0].content.items.length,
    3,
    'client-added tail item survives',
  );
...
test('applyPresentationToDoc with base: deliberate removals still apply', () => {
  ...
  next.i18n.versions['en-GB'].slides = next.i18n.versions['en-GB'].slides
    ...
    .map((s) => ({
      ...s,
      content: { ...s.content, items: (s.content.items || []).slice(0, 1) },
    }));
...
test('server-as-collaborator: facade writes reach live editors', async (t) => {
  ...
  await t.test(
    'an MCP-style server write appears live in both editors',
    async () => {
      ...
      await waitFor(
        async () =>
          (await getPresentation(testScope(), stored.id))?.slides?.[0]
            ?.notes === 'Notitie van Alice',
        { timeout: 12000 },
      );

      // Server-side mutation through the facade (the MCP/public-API shape).
      const body = structuredClone(
        await getPresentation(testScope(), stored.id),
      );
      ...
      const result = await updatePresentation(testScope(), stored.id, body, {
      ...
      await waitFor(
        () =>
          ...
          a.pres.slides.find((s) => s.id === firstId)?.content?.title ===
            'Server titel' &&
          b.pres.slides.find((s) => s.id === firstId)?.content?.title ===
            'Server titel' &&
  ...
  await t.test('client undo managers never track server writes', async () => {
    ...
    await waitFor(
      () => b.pres.slides.find((s) => s.id === firstId)?.notes === '',
    );
  ...
  await t.test(
    'no store loop: JSON revision stays put, binary kept',
    async () => {
      ...
      await sleep(3000);
      const p = await getPresentation(testScope(), stored.id);
      assert.equal(
        p.revision,
        serverRevision,
        'no extra revision bumps after the server write',
      );
      assert.equal(p.title, 'Door de server hernoemd');
      const bin = await getYDocState(testScope(), stored.id);
      assert.ok(
        bin instanceof Uint8Array && bin.length > 0,
        'binary not invalidated (doc is live)',
      );
    },
  );

  await t.test(
    'an in-flight client edit survives a same-moment server write',
    async () => {
      ...
      const bSlide = b.pres.slides.find(
        (s) => s.content?.title === 'Server slide',
      );
      ...
      await waitFor(
        () =>
          a.pres.slides.find((s) => s.id === firstId)?.content?.title ===
            'Server titel v2' &&
          a.pres.slides.find((s) => s.id === bSlide.id)?.content?.body ===
            'Bob typt hier tegelijk' &&
          b.pres.slides.find((s) => s.id === firstId)?.content?.title ===
            'Server titel v2' &&
          b.pres.slides.find((s) => s.id === bSlide.id)?.content?.body ===
            'Bob typt hier tegelijk',
      );
      ...
      await waitFor(
        async () => {
          const p = await getPresentation(testScope(), stored.id);
          return (
            p.slides.find((s) => s.id === firstId)?.content?.title ===
              'Server titel v2' &&
            p.slides.find((s) => s.id === bSlide.id)?.content?.body ===
              'Bob typt hier tegelijk'
          );
        },
        { timeout: 12000 },
  ...
  await t.test('a theme-change-style write reaches the doc', async () => {
    // The /change-theme route's write shape: the deck's `theme`, which the
    // shared write path only lets through with `allowThemeChange`.
    const body = structuredClone(await getPresentation(testScope(), stored.id));
    body.theme = 'midnight';
    const result = await updatePresentation(testScope(), stored.id, body, {
    ...
    assert.ok(result && result.ok !== false);
    await waitFor(
      () => a.pres.theme === 'midnight' && b.pres.theme === 'midnight',
    );
  ...
  await t.test(
    'a translate-style write fills another language live',
    async () => {
      ...
      await waitFor(
        async () =>
          (await getPresentation(testScope(), stored.id))?.i18n?.versions?.[
            'en-GB'
          ],
        { timeout: 12000 },
      );
      ...
      await waitFor(() => {
        ...
        return (
          proj?.slides?.find((s) => s.id === firstId)?.content?.title ===
          `EN ${nlTitle}`
        );
      });
