...
import { ROUTES as SC_ROUTES, handleSlideCollections } from '../server/routes/api/slide-collections.js';
import { ROUTES as ORG_ROUTES, handleOrganizations } from '../server/routes/api/organizations.js';
import { ROUTES as NOTIF_ROUTES, handleNotifications } from '../server/routes/api/notifications.js';
import { ROUTES as AU_ROUTES, handleAdminUsers } from '../server/routes/api/admin-users.js';
import { ROUTES as AI_ROUTES, handleAdminAiLogs } from '../server/routes/api/admin-ai-logs.js';
import { ROUTES as CST_ROUTES, handleCustomSlideTypes } from '../server/routes/api/custom-slide-types.js';
import { ROUTES as ET_ROUTES, handleEmailTemplates } from '../server/routes/api/email-templates.js';
import { ROUTES as LSA_ROUTES, handleLiveSessionsPublic } from '../server/routes/api/live-session-audience.js';
...
test('slide-collections: routes resolve to their named handlers in order', () => {
  named(SC_ROUTES, 'GET', '/api/slide-collections/personal', 'handlePersonalList');
  named(SC_ROUTES, 'POST', '/api/slide-collections/personal', 'handlePersonalCreate');
  ...
  named(SC_ROUTES, 'GET', '/api/slide-collections/team', 'handleTeamList');
  named(SC_ROUTES, 'POST', '/api/slide-collections/team', 'handleTeamCreate');
  named(SC_ROUTES, 'GET', '/api/slide-collections/team/abc', 'handleTeamGet');
  named(SC_ROUTES, 'PATCH', '/api/slide-collections/team/abc', 'handleTeamUpdate');
  named(SC_ROUTES, 'DELETE', '/api/slide-collections/team/abc', 'handleTeamDelete');
});
...
test('slide-collections: a wrong method 405s with the pinned Allow list', async () => {
  for (const [method, path, allow] of [
    ...
    ['DELETE', '/api/slide-collections/team', 'GET, POST'],
    ['POST', '/api/slide-collections/team/abc', 'GET, PATCH, DELETE'],
  ]) {
...
test('slide-collections: module guards — foreign prefix falls through, unauth 401s', async () => {
  ...
  assert.equal(unauth.res.statusCode, 401, 'no user → 401 for any slide-collections path');
});
...
test('organizations: routes resolve to their named handlers', () => {
  ...
  named(ORG_ROUTES, 'POST', '/api/organizations/org-1/switch', 'handleOrgSwitch');
});
...
test('organizations: a wrong method falls through in the table (Form A)', () => {
  ...
  assert.equal(select(ORG_ROUTES, 'GET', '/api/organizations/org-1/switch'), null);
});
...
test('organizations: module guards — foreign prefix falls through, feature flag 403s', async () => {
  ...
  assert.equal(flagged.res.statusCode, 403, 'flag disabled → 403 for any organizations path');
});
...
test('notifications: routes resolve to their named handlers in order', () => {
  named(NOTIF_ROUTES, 'GET', '/api/notifications/events', 'handleNotificationEvents');
  named(NOTIF_ROUTES, 'GET', '/api/notifications/unread-count', 'handleNotificationUnreadCount');
  named(NOTIF_ROUTES, 'POST', '/api/notifications/mark-read', 'handleNotificationMarkRead');
  named(NOTIF_ROUTES, 'POST', '/api/notifications/archive', 'handleNotificationArchive');
...
test('admin-users: routes resolve to their named handlers', () => {
  ...
  named(AU_ROUTES, 'POST', '/api/admin/users/u-1/resend-invitation', 'handleAdminUserResendInvitation');
});
...
test('admin-ai-logs: routes resolve to their named handlers', () => {
  ...
  named(AI_ROUTES, 'GET', '/api/admin/ai-logs/download/some-file.ndjson', 'handleAiLogsDownload');
...
test('custom-slide-types: routes resolve to their named handlers in order', () => {
  named(CST_ROUTES, 'GET', '/api/custom-slide-types', 'handleCustomSlideTypeList');
  named(CST_ROUTES, 'POST', '/api/custom-slide-types', 'handleCustomSlideTypeCreate');
  named(CST_ROUTES, 'PUT', '/api/custom-slide-types/reorder', 'handleCustomSlideTypeReorder');
  named(CST_ROUTES, 'POST', '/api/custom-slide-types/abc123/duplicate', 'handleCustomSlideTypeDuplicate');
  named(CST_ROUTES, 'GET', '/api/custom-slide-types/abc123', 'handleCustomSlideTypeGet');
  named(CST_ROUTES, 'PUT', '/api/custom-slide-types/abc123', 'handleCustomSlideTypeUpdate');
  named(CST_ROUTES, 'DELETE', '/api/custom-slide-types/abc123', 'handleCustomSlideTypeDelete');
});
...
test('custom-slide-types: /duplicate falls through on a wrong method (Form A)', async () => {
  assert.equal(select(CST_ROUTES, 'GET', '/api/custom-slide-types/abc123/duplicate'), null);
...
test('email-templates: routes resolve to their named handlers in order', () => {
  named(ET_ROUTES, 'GET', '/api/admin/email-templates', 'handleEmailTemplateList');
  named(ET_ROUTES, 'GET', '/api/admin/email-templates/metadata', 'handleEmailTemplateMetadata');
  named(ET_ROUTES, 'PUT', '/api/admin/email-templates/settings', 'handleEmailTemplateSettings');
  named(ET_ROUTES, 'PUT', '/api/admin/email-templates/welcome/nl', 'handleEmailTemplateWrite');
  named(ET_ROUTES, 'DELETE', '/api/admin/email-templates/welcome/nl', 'handleEmailTemplateReset');
  named(ET_ROUTES, 'POST', '/api/admin/email-templates/welcome/preview', 'handleEmailTemplatePreview');
  named(ET_ROUTES, 'POST', '/api/admin/email-templates/welcome/test', 'handleEmailTemplateTest');
});
...
test('email-templates: the :type/:locale PUT row still shadows /preview for PUTs (first-match order)', () => {
  ...
  named(ET_ROUTES, 'PUT', '/api/admin/email-templates/welcome/preview', 'handleEmailTemplateWrite');
});
...
test('live-session-audience: routes resolve to their named handlers', () => {
  named(LSA_ROUTES, 'GET', '/api/live-sessions/s-1/state', 'handleSessionState');
  named(LSA_ROUTES, 'GET', '/api/live-sessions/s-1/events', 'handleSessionEvents');
  named(LSA_ROUTES, 'GET', '/api/live-sessions/s-1/deck', 'handleSessionDeck');
  named(LSA_ROUTES, 'PUT', '/api/live-sessions/s-1/notes/slide-1', 'handleSessionNotesWrite');
});
