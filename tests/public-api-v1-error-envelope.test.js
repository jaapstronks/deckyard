...
test('sendV1Error emits { error, message } with no `ok`, code defaulting from status', () => {
  ...
  assert.ok(
    !('ok' in body),
    'the public envelope carries no `ok` discriminator',
  );
});
...
test('withV1ErrorHandler turns an unexpected throw into a generic 500, no leak, no `ok`', async () => {
  ...
  const handled = await wrapped({
    res,
    req: { method: 'GET' },
    url: new URL('http://x/api/v1/boom'),
  });
  ...
  assert.ok(
    !String(res.body).includes('secret internal detail'),
...
test('withV1ErrorHandler preserves an AppError status, machine code and details', async () => {
  const res = makeRes();
  const wrapped = withV1ErrorHandler('test', async () => {
    throw new LockedError('Slide is locked', {
      slideId: 's1',
      lockKind: 'author',
    });
  });
...
test('withV1ErrorHandler does not double-answer once headers are flushed', async () => {
  ...
  assert.equal(
    res.body,
    null,
    'no error envelope is written over a flushed stream',
  );
});
...
test('withV1ErrorHandler passes a normal return value through unchanged', async () => {
  ...
  assert.equal(
    await wrapped({ res, req: {}, url: new URL('http://x/api/v1/x') }),
    false,
  );
  assert.equal(
    res.statusCode,
    null,
    'no response written when the handler did not match',
  );
});
...
test('every feature handler mounted in the v1 router is withV1ErrorHandler-wrapped', () => {
  ...
  const entries = [...indexSrc.matchAll(re)].map((m) => ({
    name: m[1],
    file: m[2],
  }));
  assert.ok(
    entries.length >= 9,
    `expected the feature handler imports, saw ${entries.length}`,
  );
  ...
  assert.deepEqual(
    unwrapped,
    [],
    `these v1 module entries must be withV1ErrorHandler-wrapped:\n  ${unwrapped.join('\n  ')}`,
  );
...
test('a maintenance write refusal on /api/v1 answers the v1 envelope; internal keeps ok:false', async (t) => {
  ...
  assert.ok(
    !('ok' in v1Body),
    'the v1 surface never carries the internal `ok` discriminator',
  );
  ...
  assert.equal(
    internalBody.ok,
    false,
    'the internal surface keeps its own envelope',
  );
