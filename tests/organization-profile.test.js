...
globalThis.fetch = async (input, init = {}) => {
  ...
  return new Response(JSON.stringify({ ok: true, organization: ORGANIZATION }), {
    status: 200,
    headers: { 'content-type': 'application/json' },
  });
};
...
const { canEditProfile, canDeleteOrganization, isOrganizationOwner } = await import(
  '../client/views/settings/organization-profile/permissions.js'
);
const { fetchOrganization, saveOrganizationProfile, changedFields } = await import(
  '../client/views/settings/organization-profile/actions.js'
);
const { renderOrganizationProfilePanel } = await import(
  '../client/views/settings/organization-profile/panel.js'
);
...
async function renderPanel(user, { organization = ORGANIZATION, ...overrides } = {}) {
  const panel = renderOrganizationProfilePanel({
    user,
    load: async () => ({ organization, membership: { role: user.organizationRole } }),
    openDelete: () => {},
    onDeleted: () => {},
    ...overrides,
  });
...
test('the default organization cannot be deleted by anyone, its owner included', () => {
  ...
  assert.equal(isOrganizationOwner(owner), true, 'the owner section still exists');
  assert.equal(
...
test('a plain member reads the profile and cannot touch it', async () => {
  ...
  assert.match(panel.el.textContent, /Only an admin or the owner can change it/);
});
...
test('saving posts the changed field alone', async () => {
  ...
  assert.deepEqual(sent, { organizationId: ORG, updates: { displayName: 'Beta HQ' } });
...
test('the read is the route the profile screen is a mirror of', async () => {
  ...
  assert.deepEqual(requested, [{ method: 'GET', path: `/api/organizations/${ORG}`, body: null }]);

  requested = [];
  nextResponse = { status: 200, body: { ok: true, organization: ORGANIZATION } };
  await saveOrganizationProfile({ organizationId: ORG, updates: { name: 'Beta Ltd' } });
  assert.deepEqual(requested, [
    { method: 'PATCH', path: `/api/organizations/${ORG}`, body: { name: 'Beta Ltd' } },
  ]);
});
...
function openDeleteDialog({ remove, onDeleted, organization = ORGANIZATION } = {}) {
  const modal = showDeleteOrganizationModal({ organization, remove, onDeleted });
...
test('deleting reports it and hands over to the caller', async () => {
  ...
  assert.equal(document.querySelector('.organization-delete-modal'), null, 'dialog closed');
});
...
test('a refused delete stays in the dialog with the server sentence', async () => {
  ...
  assert.ok(document.querySelector('.organization-delete-modal'), 'the dialog stays open');
  assert.match(dialog.status.textContent, /default organization cannot be deleted/i);
});
