...
const updating = /^(1|true|yes)$/i.test(
  String(process.env.UPDATE_EXPORT_METRICS || '').trim(),
);
...
function boxAllowance(baselineValue) {
  return Math.max(
    Math.abs(baselineValue) * BOX_TOLERANCE_RATIO,
    BOX_TOLERANCE_FLOOR_PX,
  );
}
    ...
    async () => {
      ...
      for (const selector of MEASURED_SELECTORS) {
        ...
        assert.equal(
          actual.fontSize,
          expected.fontSize,
          `${selector}: font-size changed`,
        );
        assert.equal(
          actual.fontWeight,
          expected.fontWeight,
          `${selector}: font-weight changed`,
        );
        ...
        assert.equal(
          actual.text,
          expected.text,
          `${selector}: rendered text changed`,
        );
      }
      assert.deepEqual(
        diffs,
        [],
        `layout drifted beyond tolerance:\n  ${diffs.join('\n  ')}`,
      );
      ...
      assert.ok(
        tokenRgb,
        `${themeName}: ${tokenName} is not a parseable colour`,
      );
      for (let i = 0; i < 3; i++) {
        assert.ok(
          Math.abs(metrics.dominantColor.rgb[i] - tokenRgb[i]) <=
            COLOR_TOLERANCE,
  ...
  test(
    ...
    async () => {
      ...
      assert.deepEqual(
        metrics.parseErrors,
        [],
        'the PDF should parse without errors',
      );
      assert.equal(
        metrics.pageCount,
        baseline.pageCount,
        'one slide in → one page out',
      );
      assert.deepEqual(
        metrics.pages,
        baseline.pages,
...
test(
  'the all-field-types deck exports one page per slide, unchanged',
  { skip },
  async () => {
...
test(
  'every all-field-types slide stays inside the frame with real fonts',
  { skip },
  async () => {
    const deck = allFieldTypesDeck();
    const theme = await loadThemeAssets(repoRoot, deck.theme);
...
test(
  'no measured element paints in a system fallback font',
  { skip },
  async () => {
    ...
    for (const selector of MEASURED_SELECTORS) {
      ...
      assert.ok(
        painted.length > 0,
        `${selector}: Chrome reported no painted font at all`,
      );
      assert.ok(
