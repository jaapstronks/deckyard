...
export default {
  ...
  // No `sample`: this type is chrome (`fields: []`, `defaults: {}`) and its
  // renderer draws the theme's payoff logo without reading `content` at all,
  // so an example can only name keys nothing declares, renders or stores.
  // Exempted from the `picker-sample` companion with that reason.
};
