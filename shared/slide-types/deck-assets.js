...
/**
 * Deep-clone a JSON value, replacing any upload ref with `mapFn(ref)`. When
 * `mapFn` returns a falsy value the original ref is kept. Non-upload strings
 * and other values are copied unchanged.
 * @param {unknown} value
...
function mapValue(value, mapFn) {
  ...
  if (Array.isArray(value)) return value.map((v) => mapValue(v, mapFn));
  if (value && typeof value === 'object') {
    const out = {};
    for (const [k, v] of Object.entries(value)) out[k] = mapValue(v, mapFn);
    return out;
  }
  return value;
}
...
export function rewriteAssetRefs(deck, mapFn) {
  ...
  const slides = Array.isArray(deck.slides)
    ? deck.slides.map((slide) => ({
        ...slide,
        content: mapValue(slide?.content ?? {}, mapFn),
      }))
    : deck.slides;
  return { ...deck, slides };
}
