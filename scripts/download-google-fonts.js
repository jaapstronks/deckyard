 ...
 *
 * Two lockfiles, one per owner. A fork declares its own families in
 * `custom/fonts.js`, and their pins go to `custom/google-fonts.lock.json` —
 * written by this same run, read and merged by it, never mixed into upstream's
 * file. That is the whole B163 fix: before it, adding a family meant editing a
 * generated core file, so `npm install` failed on a clean merge. Both halves of
 * the seam now sit on the fork's side of the line, and the pin is still a pin.
 *
 * Usage:
...
/** Which lockfile owns a family's pins, spelled the way a message prints it. */
const lockRelFor = (family) =>
  isCustomFont(family)
    ? CUSTOM_FONTS_LOCK_REL
    : path.relative(ROOT_DIR, LOCK_PATH);
...
async function updateLock(options) {
  ...
  // Drop families that have left CURATED_FONTS, so neither lock carries pins
  // for fonts nothing can request any more.
  ...
  // Each family's pins go to the lock its owner controls. Upstream's file is
  // written every run; the fork's only once there is something to put in it, so
  // an OSS checkout never grows an empty `custom/` artifact.
  const coreFamilies = Object.keys(fonts).filter((f) => !isCustomFont(f));
  const customFamilies = Object.keys(fonts).filter(isCustomFont);

  await writeLock(LOCK_PATH, coreFamilies, fonts, CORE_LOCK_COMMENT);
...
const CUSTOM_LOCK_COMMENT =
  `The fork half of the font pin: the families declared in ${CUSTOM_FONTS_FILE_REL}. ` +
  'Same format and same guarantee as scripts/google-fonts.lock.json, written by ' +
  "the same `--update-lock` run, so a fork never edits upstream's copy. " +
  'Do not hand-edit.';

/**
 * Write one lockfile: the given families, sorted, with the shared subset header.
 *
 * @param {string} lockPath - absolute path to write
 * @param {string[]} families - families this file owns
 * @param {Object} fonts - family → {slug, files} for every resolved family
 * @param {string} comment - the `$comment` header explaining the file
 */
async function writeLock(lockPath, families, fonts, comment) {
  const lock = {
    $comment: comment,
    ...
    fonts: Object.fromEntries(
      [...families].sort().map((family) => [family, fonts[family]]),
    ),
  };
  await fs.mkdir(path.dirname(lockPath), { recursive: true });
  await fs.writeFile(lockPath, `${JSON.stringify(lock, null, 2)}\n`);
  console.log(`\n📄 Lockfile written: ${path.relative(ROOT_DIR, lockPath)}`);
}

/**
 * Read and shallow-validate one lockfile.
 * @param {string} lockPath - absolute path
 * @param {{optional?: boolean}} [opts]
 * @returns {Promise<Object|null>}
 */
async function readOneLock(lockPath, { optional = false } = {}) {
  const rel = path.relative(ROOT_DIR, lockPath);
  let raw;
  try {
  ...
  if (!lock?.fonts || typeof lock.fonts !== 'object') {
    throw new Error(`${rel} has no "fonts" map`);
  }
  return lock;
}

/**
 * The pins this process should honour: upstream's, plus the fork's when the
 * seam is populated.
 *
 * Merged rather than chosen between, because every consumer downstream asks one
 * question — "is this family pinned" — and should not have to know which side
 * of the seam the answer came from. The fork's file is always optional: an OSS
 * checkout has none, and a fork that has declared fonts but not resolved them
 * yet gets the pointed "not in the lockfile" message below rather than a crash
 * on a missing file.
 *
 * @param {{optional?: boolean}} [opts] - applies to upstream's lock
 * @returns {Promise<Object|null>}
 */
async function readLock({ optional = false } = {}) {
  const core = await readOneLock(LOCK_PATH, { optional });
  const custom = await readOneLock(CUSTOM_FONTS_LOCK_PATH, { optional: true });
  if (!core) return custom;
  if (!custom) return core;
  return { ...core, fonts: { ...core.fonts, ...custom.fonts } };
}
